    ...
    styles: [
        `
           .custom-layout {
            ...
            .redemption_child_btn {
                display: flex;
                gap: 10px;
            }

            .redemption_successMessage_scss {
                position: unset;
                text-align: center;
                color: rgb(0, 128, 0);
                padding: 3rem;
        ...
        .subscribe(() => {
            ...
            (response) => {
                if (response.status === 200) {
                    ...
                    this._router.navigate(['/redemption'], { relativeTo: this._activatedRoute });
                    this.isSuccess = true;
                    this._changeDetectorRef.markForCheck();
                } else {
                    // Error response
