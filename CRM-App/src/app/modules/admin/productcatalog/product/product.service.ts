...
export class ProductService {
    ...
    getProducts(page: number = 0, limit: number = 10, sort: string = 'item_number', order: 'asc' | 'desc' | '' = 'asc', search: string = ''):
        ...
        return this._httpClient.get(`${this._apiurl}/items/product_exclusion`, {
            params: {
                meta: 'filter_count',
                page: page + 1,
                limit: limit,
    ...
    getProductById(id: number): Observable<Product> {
        return this._httpClient.get<any>(`${this._apiurl}/items/product_exclusion/${id}`)
            .pipe(
                tap((response) => {
    ...
    updateProduct(id: number, product: Product): Observable<Product> {
        return this._httpClient.patch<Product>(`${this._apiurl}/items/product_exclusion/${id}`, {
            'id': product.id,
            'status': product.status,
            'item_name': product.item_name,
            'price': product.price,
